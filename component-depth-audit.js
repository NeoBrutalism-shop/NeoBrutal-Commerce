...
async function initComponentDepthAudit(){
  try{
    ...
    depthAssert(batches.length===3,'component demo depth must expose three implementation batches');
    depthAssert(batches.map(batch=>batch.ordinal).join(',')==='1,2,3','component implementation batch ordinals drifted');
    ...
    depthAssert(implementationById.size===26,'component demo implementation must cover exact accumulated 26-component evidence set');
