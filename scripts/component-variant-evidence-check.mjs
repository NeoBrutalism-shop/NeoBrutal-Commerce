...
for(const marker of ['100','56','36','4','2','data-component-variant-evidence','data-variant-choice','data-variant-current','data-variant-state-ref','data-variant-responsive-ref','data-variant-interaction-ref','data-variant-action-ref','data-responsive-mode','data-responsive-viewport','data-interaction-id','data-action-id','AxeBuilder','aria-pressed','cart-item','order-summary','coupon','checkout-field','checkout-steps','payment-method','payment-failure','payment-recovery','processing-state','order-confirmation','receipt','download-entitlement'])if(!browser.includes(marker))fail(`Variant evidence Browser QA missing current combined marker: ${marker}`);

console.log(`Component variant evidence passed · certified four-batch base ${componentIds.length}/47 components · ${totalVariants} exact variants · ${renderedVariants} rendered / ${stateBackedVariants} canonical-state-backed / ${responsiveBackedVariants} responsive-backed / ${interactionBackedVariants} interaction-backed / ${actionBackedVariants} action-backed · base depth 28 complete / 19 partial`);