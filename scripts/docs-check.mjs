...
if(depthCounts.variants.complete!==16||depthCounts.variants.partial!==31)fail('Variant audit must remain exact at 16 complete / 31 partial for the accumulated storefront + Product/trust + Pricing proof batches');
...
for(const marker of ['37','32','3','2','data-component-variant-evidence','data-variant-choice','data-variant-current','data-variant-state-ref','data-variant-responsive-ref','data-responsive-mode','data-responsive-viewport','data-showcase-state','data-media-state','AxeBuilder','aria-pressed','product-card','trust-strip','promo-band','badge','price-block','product-detail','product-gallery','product-media','review-summary','testimonials','guarantee','license-selector','renewal-note','pricing-tier','plan-comparison','bundle-builder'])if(!variantBrowser.includes(marker))fail(`Component variant evidence Browser QA missing marker: ${marker}`);
...
for(const marker of ['v1.1 Component demo depth audit','audit evidence only','47','12','43','4','16','All variants','31','37','32','3','2','canonical-state','responsive-backed','Design tokens used','Copy-ready HTML / CSS / JS','First implementation-depth batch','Second implementation-depth batch','Third implementation-depth batch','Fourth implementation-depth batch','Fifth implementation-depth batch','First variant-evidence batch','Second variant-evidence batch','Third variant-evidence batch','Product + trust','Pricing','contained-scroll','storefront/component-demo-depth.json','storefront/component-variant-evidence.json','storefront/component-demo-implementation.json','storefront/component-demo-implementation-product.json','storefront/component-demo-implementation-trust-pricing.json','storefront/component-demo-implementation-cart-checkout.json','storefront/component-demo-implementation-account-ownership.json'])if(!depthDoc.includes(marker))fail(`Component demo depth audit doc missing marker: ${marker}`);
...
console.log(`NeoBrutal Commerce v1.0 adoption docs + Showcase v1.1 passed · ${manifest.components.length} components · ${blocks.blocks.length} blocks · ${documentedStates} live states · ${routes.routes.length} production routes · component demo depth audit ${depthComponents.length}/${manifest.components.length} · implementation evidence ${implementationIds.length}/47 · variant evidence 16/47`);
