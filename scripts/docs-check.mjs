...
if(depthCounts.variants.complete!==13||depthCounts.variants.partial!==34)fail('Variant audit must remain exact at 13 complete / 34 partial for the accumulated storefront + Product/trust proof batches');
...
for(const marker of ['v1.1 Component demo depth audit','audit evidence only','47','12','43','4','16','All variants','13','34','31','28','3','canonical-state','Design tokens used','Copy-ready HTML / CSS / JS','First implementation-depth batch','Second implementation-depth batch','Third implementation-depth batch','Fourth implementation-depth batch','Fifth implementation-depth batch','First variant-evidence batch','Second variant-evidence batch','Product + trust','storefront/component-demo-depth.json','storefront/component-variant-evidence.json','storefront/component-demo-implementation.json','storefront/component-demo-implementation-product.json','storefront/component-demo-implementation-trust-pricing.json','storefront/component-demo-implementation-cart-checkout.json','storefront/component-demo-implementation-account-ownership.json'])if(!depthDoc.includes(marker))fail(`Component demo depth audit doc missing marker: ${marker}`);
...
console.log(`NeoBrutal Commerce v1.0 adoption docs + Showcase v1.1 passed · ${manifest.components.length} components · ${blocks.blocks.length} blocks · ${documentedStates} live states · ${routes.routes.length} production routes · component demo depth audit ${depthComponents.length}/${manifest.components.length} · implementation evidence ${implementationIds.length}/47 · variant evidence 13/47`);
