...
const required=[
  ...
  'storefront/component-demo-implementation.json','storefront/component-demo-implementation-product.json','storefront/component-demo-implementation-trust-pricing.json','storefront/component-demo-implementation-cart-checkout.json','storefront/blocks.json',
...
if(batches.length!==4||batches.map(batch=>batch.ordinal).join(',')!=='1,2,3,4')fail('Component demo implementation batch lineage must contain ordered batches 1, 2, 3, and 4');
...
if(implementationIds.length!==33||new Set(implementationIds).size!==33)fail('Accumulated implementation evidence batch membership must cover 33 unique components');
...
if(depthCounts.tokens.complete!==33||depthCounts.tokens.missing!==14||depthCounts['copy-ready'].complete!==33||depthCounts['copy-ready'].missing!==14)fail('Accumulated demo implementation evidence must remain exact at 33 complete / 14 missing for token and copy-ready criteria');
...
for(const marker of ['47*13','data-demo-depth-first-batch','data-demo-depth-batch','Canonical states','Action contracts','Design tokens used','Copy-ready HTML / CSS / JS','AxeBuilder','33','trust-review-pricing','cart-checkout'])if(!depthBrowser.includes(marker))fail(`Component demo depth Browser QA missing marker: ${marker}`);
...
for(const marker of ['v1.1 Component demo depth audit','audit evidence only','47','12','33','14','16','Design tokens used','Copy-ready HTML / CSS / JS','First implementation-depth batch','Second implementation-depth batch','Third implementation-depth batch','Fourth implementation-depth batch','storefront/component-demo-depth.json','storefront/component-demo-implementation.json','storefront/component-demo-implementation-product.json','storefront/component-demo-implementation-trust-pricing.json','storefront/component-demo-implementation-cart-checkout.json'])if(!depthDoc.includes(marker))fail(`Component demo depth audit doc missing marker: ${marker}`);
