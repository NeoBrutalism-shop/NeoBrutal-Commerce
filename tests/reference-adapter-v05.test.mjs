...
test('reference licensing adapter keeps activation seat renewal and download behavior',async()=>{
  ...
  const signed=await licensing.createSignedDownload({entitlementId:source.id,releaseId:'v1.0.0'});assert.match(signed.url,/v1\.0\.0/);assert.equal(signed.releaseId,'v1.0.0');
});

test('reference runtime composes replaceable commerce and licensing adapters',async()=>{
  const runtime=createReferenceRuntime();assert.equal(runtime.version,'1.0.0');assert.equal(runtime.commerce.kind,'commerce');assert.equal(runtime.licensing.kind,'licensing');
