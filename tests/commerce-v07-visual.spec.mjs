...
test('v0.7 canonical visual fingerprints remain stable',async({page},testInfo)=>{
  test.skip(process.platform!==baseline.platform,`Canonical v0.7 fingerprints are ${baseline.platform} CI baselines.`);
  ...
  for(const surface of baseline.surfaces){
    ...
    const screenshot=await page.screenshot({
      path:testInfo.outputPath(`commerce-v07-lock-${surface.id}-${testInfo.project.name}.png`),
      fullPage:true
    });
    ...
    expect(actualHash,`${testInfo.project.name}/${surface.id} pixels drifted from reviewed v0.7 baseline`).toBe(expected.sha256);
  }
});
