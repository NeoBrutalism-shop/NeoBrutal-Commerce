...
test('capture v0.9 RC canonical visual candidates',async({page},testInfo)=>{
  ...
  for(const [id,route] of surfaces){
    ...
    await page.screenshot({path:testInfo.outputPath(`commerce-v09-candidate-${id}-${testInfo.project.name}.png`),fullPage:true});
  }
});
