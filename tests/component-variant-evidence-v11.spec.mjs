...
test('v1.1 plan-comparison variant evidence reuses one semantic table across desktop and narrow contained-scroll views',async({page})=>{
  ...
  const routeWrap=page.locator('.nbc-compare-wrap').first();
  const routeTable=routeWrap.locator('table.nbc-compare');
  await expect(routeWrap).toBeVisible();
  await expect(routeWrap).toHaveAttribute('tabindex','0');
  await expect(routeTable).toBeVisible();
  await expect(routeTable.locator('thead th')).toHaveCount(4);
  await routeWrap.focus();
  await expect(routeWrap).toBeFocused();
  await page.setViewportSize({width:360,height:800});
  await expect(routeWrap).toBeVisible();
  await expect(routeTable).toBeVisible();
  const narrowGeometry=await routeWrap.evaluate(element=>({scrollWidth:element.scrollWidth,clientWidth:element.clientWidth}));
