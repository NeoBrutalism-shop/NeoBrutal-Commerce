...
export const COMMERCE_REQUIRED_METHODS=freezeList([
  'listProducts',
  'getProduct',
  'getCart',
  'addCartLine',
...
export const LICENSING_REQUIRED_METHODS=freezeList([
  'listLicenses',
  'getLicense',
  'listEntitlements'
]);
...
function assertMethods(value,methods,label){
  const missing=methods.filter(method=>!hasOwnMethod(value,method));
  if(missing.length){
    throw new TypeError(`${label} is missing required method${missing.length===1?'':'s'}: ${missing.join(', ')}`);
  }
}

function normalizeCapabilities(input,defaults={}){
  if(input==null)return Object.freeze({...defaults});
  assertObject(input,'Adapter capabilities');
  return Object.freeze({...defaults,...input});
}

function makeStateGuard(values){
...
export function assertKnownState(group,value){
  ...
  if(!values.includes(value)){
    throw new TypeError(`Unknown ${group} state: ${String(value)}. Expected one of: ${values.join(', ')}`);
  }
  return value;
}

export function createCommerceAdapter(adapter){
  assertObject(adapter,'Commerce adapter');
  assertMethods(adapter,COMMERCE_REQUIRED_METHODS,'Commerce adapter');
...
export function createLicensingAdapter(adapter){
  assertObject(adapter,'Licensing adapter');
  assertMethods(adapter,LICENSING_REQUIRED_METHODS,'Licensing adapter');
  const capabilities=normalizeCapabilities(adapter.capabilities,{
    activations:false,
    seats:false,
    renewals:false,
